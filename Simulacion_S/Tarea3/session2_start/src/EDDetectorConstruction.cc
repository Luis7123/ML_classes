...
{  
  ...
  new G4PVPlacement(0,
                    G4ThreeVector(0,0,2.*m),       //at (0,0,0)
  ...
  G4double layerPosition = -hx + 15.*cm ;
  G4double dx = 0.1*cm;
  for (G4int i=0; i<20; ++i) {
    G4Box* solidLayer = new G4Box("Layer", 15, hy, hz);
    G4LogicalVolume* logicLayer = new G4LogicalVolume(solidLayer, CsI, "Layer_LV");

  new G4PVPlacement(0,
                    G4ThreeVector(layerPosition, 0, 0),
                    logicLayer,
                    "Layer_PV",
                    calorimeterLV,
                    false,
 ...
 logicLayer->SetVisAttributes(calorimeterVisAtt);
